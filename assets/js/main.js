...
(() => {
  ...
  document.querySelectorAll('[data-ajax-form]').forEach(form=>form.addEventListener('submit',async e=>{e.preventDefault();const msg=form.querySelector('.form-message');const btn=form.querySelector('button[type="submit"]');msg.textContent='';btn.disabled=true;const old=btn.innerHTML;btn.textContent='Submitting...';try{const res=await fetch(form.action,{method:'POST',body:new FormData(form),headers:{'Accept':'application/json'},credentials:'same-origin'});const raw=await res.text();let data;try{data=JSON.parse(raw);}catch(parseError){throw new Error(res.ok?'The server returned an invalid response. Check storage/logs and PHP error logs.':'Server error '+res.status+'. Check the form handler and PHP error log.');}if(!res.ok||!data.ok)throw new Error(data.message||'Submission failed.');msg.className='form-message success-text';msg.textContent='Submitted successfully. Redirecting...';window.location.assign(data.redirect);}catch(err){msg.className='form-message error-text';msg.textContent=err.message||'Something went wrong. Please call us.';btn.disabled=false;btn.innerHTML=old;}}));
